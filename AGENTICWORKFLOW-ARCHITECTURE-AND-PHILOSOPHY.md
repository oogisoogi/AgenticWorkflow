...
절대 기준 1(품질)과 절대 기준 2(SOT)가 충돌하면, **품질이 이긴다**. SOT는 품질을 보장하기 위한 **수단**이지, 품질을 제약하는 **목적**이 아니다.

```mermaid
flowchart TD
    Q["설계/구현 의사결정 발생"]
    Q --> C1{"절대 기준 1에 의한<br/>품질 판단은?"}
    C1 -->|"이 선택이 품질↑"| C2{"절대 기준 2(SOT)<br/>와 충돌하는가?"}
    C1 -->|"이 선택이 품질↓"| REJECT["기각: 품질 저하 선택 불가"]
    C2 -->|"충돌 없음"| ADOPT["채택: 품질↑ + SOT 준수"]
    C2 -->|"충돌 있음"| RESOLVE{"충돌 해소 유형"}
    RESOLVE -->|"SOT 병목 →<br/>에이전트가 stale data 사용"| FIX1["산출물 직접 참조 허용<br/>(SOT 구조 조정)"]
    RESOLVE -->|"품질 향상이<br/>SOT 복잡도 증가 유발"| FIX2["감수<br/>(품질 > SOT)"]
    RESOLVE -->|"에이전트 간<br/>공유 상태가 아예 없음"| FIX3["SOT 경량화 허용<br/>(판단 근거 문서화)"]
    FIX1 --> GUARD["단, SOT 파일 자체의<br/>단일 쓰기 지점은 보존"]
    FIX2 --> GUARD
    FIX3 --> GUARD
...
| 시나리오 | 해소 방법 | 보존하는 것 |
|---------|----------|------------|
| Team Lead가 유일한 SOT 쓰기 지점이라 팀원이 stale data로 작업 | 팀원 간 산출물 파일 직접 참조 허용 | SOT 파일 자체의 단일 쓰기 지점 |
| 품질 향상을 위한 단계 추가가 SOT 상태 복잡도를 증가시킴 | 복잡도 증가를 감수 | 절대 기준 1 우선 원칙 |
| 완전 독립 병렬 작업에서 SOT가 불필요 | SOT 경량화 허용 | 판단 근거를 워크플로우에 명시 |
...
| **1>2 충돌** | SOT 구조가 품질 병목이면 조정 가능 | 일관성이 정확성을 해치면 정확성 우선 → 소급 일관성 업데이트 |

이 맥락화가 스킬 개발 규칙의 첫 번째 항목("절대 기준 3개를 반드시 포함 — 해당 도메인에 맞게 맥락화")이 존재하는 이유이다.
...
| 특성 | Sub-agent | Agent Team | Hook | Slash Command |
|------|-----------|------------|------|---------------|
| **세션** | 단일 (위임) | 다중 (독립) | N/A | N/A |
| **컨텍스트** | 부모와 분리 | 완전 독립 | 없음 | 없음 |
| **통신** | 결과만 반환 | 양방향 메시징 | 단방향 (exit code) | 사용자↔시스템 |
| **SOT 쓰기** | Orchestrator가 반환값으로 갱신 | **Team Lead만** 갱신 | 읽기 전용 (검증만) | 사용자 입력 반영 |
...
| # | 의무 사항 | 근거 |
|---|---------|------|
| 1 | **절대 기준 3개를 반드시 포함** — 해당 도메인에 맞게 맥락화 | 스킬이 프로젝트의 가치 체계와 정렬되어야 함 |
